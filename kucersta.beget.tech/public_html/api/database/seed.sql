...
INSERT INTO partners (name, description, website_url, logo_url, sort_order, is_published) VALUES
('АО РЕШЕТНЁВ', 'Базовый индустриальный партнер по стажировкам.', 'https://www.iss-reshetnev.ru/', '/api/public/uploads/partners/reshetnev.png', 1, 1),
('Красмаш', 'Партнер по практикам и трудоустройству.', 'https://krasm.com/', '/api/public/uploads/partners/krasmash.png', 2, 1),
('Россети Сибирь', 'Партнер по инженерным специальностям.', 'https://www.rosseti-sib.ru/', '/api/public/uploads/partners/rosseti.png', 3, 1);

-- ВНИМАНИЕ: пути '/uploads/demo/*.jpg' ниже являются демо-плейсхолдерами.
-- Реальные файлы клиент должен загрузить через админку (cover/баннеры),
-- после чего пути автоматически примут вид '/api/public/uploads/img_*.png'.
-- До загрузки клиент будет показывать заглушку — это ожидаемое поведение.
INSERT INTO media_assets (title, media_type, audience, url, alt_text, sort_order, is_published)
VALUES
('Главный баннер абитуриента', 'image', 'applicant', NULL, 'Абитуриентам', 1, 1),
('Баннер студентам', 'image', 'student', NULL, 'Студентам', 1, 1);

INSERT INTO pages (slug, title, audience, content_json, cover_image_url, is_published)
VALUES
('about-college', 'О колледже', 'applicant', JSON_OBJECT('lead', 'Современный аэрокосмический колледж', 'body', 'Подготовка востребованных специалистов для индустрии.'), NULL, 1),
('student-home', 'Главная для студентов', 'student', JSON_OBJECT('lead', 'Личный кабинет студента', 'body', 'Расписание, профиль, портфолио и вакансии.'), NULL, 1);
...
SELECT u.id, 'Мобильное приложение колледжа', 'Прототип приложения с API интеграцией.', 'https://github.com/example/college-app', NULL, 1, 1
FROM users u
WHERE u.email = 'student@aksibgu.local'
LIMIT 1;
