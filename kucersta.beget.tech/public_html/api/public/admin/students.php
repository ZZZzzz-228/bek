...
<div class="card">
  ...
  <?php if (!$showCreate): ?>
    <a class="btn" href="/api/public/admin/students.php?create=1">Открыть форму</a>
  <?php else: ?>
    <form method="post">
      <?= csrfField() ?>
...
<div class="card">
  ...
  <form method="get" class="filters" style="margin-bottom:12px;">
    ...
    <?php if ($q !== ''): ?>
      <div class="field">
        <label>&nbsp;</label>
        <a class="btn btnGhost" href="/api/public/admin/students.php">Сбросить</a>
      </div>
    <?php endif; ?>
    <?php if (hasRole('admin')): ?>
      <div class="field">
        <label>&nbsp;</label>
        <a class="btn btnAccent" href="/api/public/admin/students_export.php?<?= h(http_build_query(['q' => $q])) ?>">Экспорт CSV</a>
      </div>
    <?php endif; ?>
  </form>
    ...
    <table>
      ...
      <?php foreach ($rows as $row): ?>
        <tr>
          ...
          <td>
            <a href="/api/public/admin/student_view.php?id=<?= (int)$row['id'] ?>">Профиль</a>
            <?php if (hasRole('admin')): ?>
