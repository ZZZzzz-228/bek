...
if ($method === 'GET' && $path === '/contacts') {
    $stmt = $pdo->query(
        'SELECT id, type, value, label, image_url
         FROM contacts
         WHERE is_active = 1
...
if ($method === 'GET' && $path === '/stories') {
    // Поддержка нескольких фото в одной истории через колонку images_json.
    // Если миграция ещё не применена — мягко падаем на старое поведение.
    try {
        $stmt = $pdo->query(
            'SELECT id, title, content, image_url, images_json, sort_order
             FROM stories
             WHERE is_published = 1
               AND (publish_from IS NULL OR publish_from <= NOW())
               AND (publish_to IS NULL OR publish_to >= NOW())
