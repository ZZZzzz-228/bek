...
// Добавляем колонку image_url если её нет (миграция на лету)
try {
    $pdo->query('SELECT image_url FROM contacts LIMIT 1');
} catch (Throwable $e) {
    $pdo->exec('ALTER TABLE contacts ADD COLUMN image_url VARCHAR(512) NULL AFTER label');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        // Сохраняем картинку — точно так же как в staff.php
        ...
        if ($id > 0) {
            $stmt = $pdo->prepare(
                'UPDATE contacts
                 SET type=:type, value=:value, label=:label, image_url=:image_url,
            ...
            auditLog($pdo, 'update', 'contact', (string)$id, [
                'type' => $type, 'value' => $value, 'is_active' => $isActive,
            ]);
            flash('Контакт обновлен.');
        } else {
            $stmt = $pdo->prepare(
                'INSERT INTO contacts(type, value, label, image_url, sort_order, is_active)
                 VALUES (:type, :value, :label, :image_url, :sort_order, :is_active)'
            );
            $stmt->execute([
                'type'      => $type,
                'value'     => $value,
...
<div class="card">
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <label>Картинка (URL вручную)</label>
    <input name="image_url" value="<?= h((string)($editItem['image_url'] ?? '')) ?>" placeholder="https://... или оставьте пустым">
    ...
    <?php if (!empty($editItem['image_url'])): ?>
      <div style="margin-top:8px;">
        <img src="<?= h((string)$editItem['image_url']) ?>" alt="" style="max-height:80px;border-radius:6px;border:1px solid #e5e7eb;">
        <div style="font-size:.8rem;color:#6b7280;margin-top:4px;">Текущая картинка</div>
      </div>
    <?php endif; ?>
...
<div class="card">
  ...
  <table>
    <thead>
      <tr><th>ID</th><th>Фото</th><th>Тип</th><th>Значение</th><th>Label</th><th>Порядок</th><th>Статус</th><th>Действия</th></tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td>
          <?php if (!empty($row['image_url'])): ?>
            <img src="<?= h((string)$row['image_url']) ?>" alt="" style="width:40px;height:40px;object-fit:cover;border-radius:6px;">
          <?php else: ?>
        ...
        <td>
          <a href="/api/public/admin/contacts.php?edit=<?= (int)$row['id'] ?>">Редактировать</a>
          <?php if ($canDelete): ?>
